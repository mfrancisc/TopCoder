...
vector <string>	items;
		...
		__in >> __expected >> costs >> prices >> sales >> items;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << costs << ", " << prices << ", " << sales << ", " << items << std::endl;
		std::cout.flush();

		__timer_start();

		MostProfitable __object;
		string __actual = __object.bestItem(costs, prices, sales, items);
