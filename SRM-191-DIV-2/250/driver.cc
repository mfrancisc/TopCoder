...
int	finalResult;
		...
		__in >> __expected >> amounts >> centsPerDollar >> finalResult;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << amounts << ", " << centsPerDollar << ", " << finalResult << std::endl;
		std::cout.flush();

		__timer_start();

		BettingMoney __object;
		int __actual = __object.moneyMade(amounts, centsPerDollar, finalResult);
