...
string	deck;
		...
		__in >> __expected >> numPlayers >> deck;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << numPlayers << ", " << deck << std::endl;
		std::cout.flush();

		__timer_start();

		CardCount __object;
		vector <string> __actual = __object.dealHands(numPlayers, deck);
