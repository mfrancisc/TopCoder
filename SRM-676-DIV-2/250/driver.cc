...
int	budget;
		...
		__in >> __expected >> time >> cost >> budget;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << time << ", " << cost << ", " << budget << std::endl;
		std::cout.flush();

		__timer_start();

		FarmvilleDiv2 __object;
		int __actual = __object.minTime(time, cost, budget);
