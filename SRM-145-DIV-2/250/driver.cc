...
vector <string>	screen;
		...
		__in >> __expected >> dithered >> screen;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << dithered << ", " << screen << std::endl;
		std::cout.flush();

		__timer_start();

		ImageDithering __object;
		int __actual = __object.count(dithered, screen);
