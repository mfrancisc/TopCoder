...
vector <string>	scoreSheet;
		...
		__in >> __expected >> parValues >> scoreSheet;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << parValues << ", " << scoreSheet << std::endl;
		std::cout.flush();

		__timer_start();

		GolfScore __object;
		int __actual = __object.tally(parValues, scoreSheet);
