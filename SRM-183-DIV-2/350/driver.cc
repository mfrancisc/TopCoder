...
int	next;
		...
		__in >> __expected >> maxAdd >> goal >> next;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << maxAdd << ", " << goal << ", " << next << std::endl;
		std::cout.flush();

		__timer_start();

		CountGame __object;
		int __actual = __object.howMany(maxAdd, goal, next);
