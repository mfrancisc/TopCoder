...
string	current;
		...
		__in >> __expected >> initial >> current;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << initial << ", " << current << std::endl;
		std::cout.flush();

		__timer_start();

		TireRotation __object;
		int __actual = __object.getCycle(initial, current);
