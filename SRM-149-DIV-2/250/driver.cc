...
int	cents;
		...
		__in >> __expected >> dollars >> cents;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << dollars << ", " << cents << std::endl;
		std::cout.flush();

		__timer_start();

		FormatAmt __object;
		string __actual = __object.amount(dollars, cents);
