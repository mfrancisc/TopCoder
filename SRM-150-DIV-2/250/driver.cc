...
int	numPerDay;
		...
		__in >> __expected >> arrivals >> numPerDay;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << arrivals << ", " << numPerDay << std::endl;
		std::cout.flush();

		__timer_start();

		WidgetRepairs __object;
		int __actual = __object.days(arrivals, numPerDay);
