...
int	totalWidth;
		...
		__in >> __expected >> maxHeight >> minWidth >> totalHeight >> totalWidth;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << maxHeight << ", " << minWidth << ", " << totalHeight << ", " << totalWidth << std::endl;
		std::cout.flush();

		__timer_start();

		Stairs __object;
		int __actual = __object.designs(maxHeight, minWidth, totalHeight, totalWidth);
