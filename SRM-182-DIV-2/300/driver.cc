...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		vector <int>	predictedGrades;
vector <int>	actualGrades;

		__in >> __expected >> predictedGrades >> actualGrades;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << predictedGrades << ", " << actualGrades << std::endl;
		std::cout.flush();

		__timer_start();

		IBEvaluator __object;
		vector <int> __actual = __object.getSummary(predictedGrades, actualGrades);
