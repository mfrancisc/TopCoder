...
int	itemZ;
		...
		__in >> __expected >> boxX >> boxY >> boxZ >> itemX >> itemY >> itemZ;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << boxX << ", " << boxY << ", " << boxZ << ", " << itemX << ", " << itemY << ", " << itemZ << std::endl;
		std::cout.flush();

		__timer_start();

		BoxLoader __object;
		int __actual = __object.mostItems(boxX, boxY, boxZ, itemX, itemY, itemZ);
