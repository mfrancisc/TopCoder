...
int	raceDistance;
		...
		__in >> __expected >> d1 >> t1 >> d2 >> t2 >> raceDistance;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << d1 << ", " << t1 << ", " << d2 << ", " << t2 << ", " << raceDistance << std::endl;
		std::cout.flush();

		__timer_start();

		RaceApproximator __object;
		string __actual = __object.timeToBeat(d1, t1, d2, t2, raceDistance);
