...
string	cutoff;
		...
		__in >> __expected >> num >> cutoff;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << num << ", " << cutoff << std::endl;
		std::cout.flush();

		__timer_start();

		CutoffRounder __object;
		int __actual = __object.round(num, cutoff);
