...
int	current;
		...
		__in >> __expected >> distances >> speeds >> current;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << distances << ", " << speeds << ", " << current << std::endl;
		std::cout.flush();

		__timer_start();

		Swimmers __object;
		vector <int> __actual = __object.getSwimTimes(distances, speeds, current);
