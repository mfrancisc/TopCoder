...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> gameLog;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << gameLog << std::endl;
		std::cout.flush();

		__timer_start();

		ScoringEfficiency __object;
		double __actual = __object.getPointsPerShot(gameLog);
