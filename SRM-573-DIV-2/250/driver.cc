...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> altitude;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << altitude << std::endl;
		std::cout.flush();

		__timer_start();

		SkiResortsEasy __object;
		int __actual = __object.minCost(altitude);
