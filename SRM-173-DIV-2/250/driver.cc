...
int	tasksCompleted;
		...
		__in >> __expected >> taskTimes >> tasksCompleted;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << taskTimes << ", " << tasksCompleted << std::endl;
		std::cout.flush();

		__timer_start();

		ProgressBar __object;
		string __actual = __object.showProgress(taskTimes, tasksCompleted);
