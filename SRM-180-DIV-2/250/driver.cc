...
int	femaleNum;
		...
		__in >> __expected >> tankVolume >> maleNum >> femaleNum;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << tankVolume << ", " << maleNum << ", " << femaleNum << std::endl;
		std::cout.flush();

		__timer_start();

		DinkyFish __object;
		int __actual = __object.monthsUntilCrowded(tankVolume, maleNum, femaleNum);
