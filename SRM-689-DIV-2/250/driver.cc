...
#include "SimilarUserDetection.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> handles;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << handles << std::endl;
		std::cout.flush();

		__timer_start();

		SimilarUserDetection __object;
		string __actual = __object.haveSimilar(handles);
