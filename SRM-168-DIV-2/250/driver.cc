...
int	stairsPerStride;
		...
		__in >> __expected >> flights >> stairsPerStride;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << flights << ", " << stairsPerStride << std::endl;
		std::cout.flush();

		__timer_start();

		StairClimb __object;
		int __actual = __object.stridesTaken(flights, stairsPerStride);
