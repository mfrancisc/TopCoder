...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> items;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << items << std::endl;
		std::cout.flush();

		__timer_start();

		ProfitCalculator __object;
		int __actual = __object.percent(items);
