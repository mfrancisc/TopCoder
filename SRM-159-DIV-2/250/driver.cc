...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> street;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << street << std::endl;
		std::cout.flush();

		__timer_start();

		StreetParking __object;
		int __actual = __object.freeParks(street);
