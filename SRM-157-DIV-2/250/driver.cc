...
int	answer;
		...
		__in >> __expected >> upper >> answer;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << upper << ", " << answer << std::endl;
		std::cout.flush();

		__timer_start();

		GuessTheNumber __object;
		int __actual = __object.noGuesses(upper, answer);
