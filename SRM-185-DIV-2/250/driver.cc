...
vector <int>	pointsPossible;
int	finalExam;

		__in >> __expected >> pointsEarned >> pointsPossible >> finalExam;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << pointsEarned << ", " << pointsPossible << ", " << finalExam << std::endl;
		std::cout.flush();

		__timer_start();

		PassingGrade __object;
		int __actual = __object.pointsNeeded(pointsEarned, pointsPossible, finalExam);
