...
int	k;
		...
		__in >> __expected >> dna >> k;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << dna << ", " << k << std::endl;
		std::cout.flush();

		__timer_start();

		TandemRepeats __object;
		int __actual = __object.maxLength(dna, k);
